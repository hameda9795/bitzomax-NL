...
$angularJsonPath = Join-Path $PSScriptRoot "frontend\angular.json"

# بررسی وجود فایل
if (!(Test-Path $angularJsonPath)) {
    Write-Host "خطا: فایل angular.json در مسیر $angularJsonPath پیدا نشد." -ForegroundColor Red
    exit 1
}
...
Write-Host "cd src/app/services" -ForegroundColor Yellow
Write-Host "sed -i 's|import { environment } from ../../../environments/environment|import { environment } from ../../environments/environment|g' *.ts" -ForegroundColor Yellow
Write-Host "cd /opt/bitzomax-git/frontend" -ForegroundColor Yellow
